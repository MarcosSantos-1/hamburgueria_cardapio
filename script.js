...
//fechar Modal pelo botao de fechar
closeModalBtn.addEventListener("click", function(){
    cartModal.style.display = "none"
})

menu.addEventListener("click", function(event){
    //procurar a classe dentro dos parenteses:
    let parentButtom = event.target.closest(".add-to-cart-btn")
    if(parentButtom){
        const name = parentButtom.getAttribute("data-name")
        const price = parentButtom.getAttribute("data-price")
...
function addToCart(name,price){
    const existingItem = cart.find(item => item.name === name ) //-> o método find vai percorrer o array e verificar se o valor é unico   
    
    if(existingItem){   //-> se o item já existe, aumenta apenas a quantitdade +1
...
checkoutBtn.addEventListener("click",function(){
    ...
    Toastify({  text: "Ops... O estamos fechado no momento!",
    duration: 3000,
    close: true,
